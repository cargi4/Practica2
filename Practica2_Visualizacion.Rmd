...
grafico3 <- grafico2 + ggtitle("Representación de las temperaturas mínima y máxima en Australia")
grafico3

ggsave(filename = "./figure/Ejercicio1.pdf")
```
...
grafico5
grafico6 <- ggplot(ds, aes(x = Temp9am, y = Pressure9am, colour = RainToday)) + geom_point()
grafico6
#Se observa mucho mejor con la representación por color, puesto que con la forma muchas veces no se diferencia cuál es cuál y no se ve la diferencia a simple vista, al contrario que con el color.
grafico6 <- grafico6 + ggtitle("Representación de la temperatura con respecto a la presión a las 9 de la mañana en Australia.")

ggsave(filename = "./figure/Ejercicio2.pdf")
```
...
#Se comparan mucho mejor con la disposición en filas, ya que puestos por columnas se achatan y no permiten ver bien las gráficas, y además se ve peor la comparación entre la longitud y la anchura.

iris.grafico5 <- ggplot(iris.tidy, aes(x = Species, y = Value, colour = Part)) + geom_jitter()
iris.grafico5

#Se observa que el gráfico ya organiza los puntos dependiendo de las especies.

ggsave(filename = "./figure/Ejercicio3.pdf")
```
...
ej4.graf1 <- ggplot(DatosEjer4, aes(x = Fecha, y = Ventas, colour = Fabricante)) + geom_point()
ej4.graf1

#Sí, se pueden ver las tendencias de ventas y su evolución a lo largo de los años.

ej4.graf2 <- ej4.graf1 + geom_line()
ej4.graf2

ej4.graf3 <- ej4.graf1 + geom_line(aes(linetype = Fabricante))
ej4.graf3

ej4.graf3 <- ej4.graf3 + ggtitle("Ventas mensuales de diferentes compañías tecnológicas")
ej4.graf3

ggsave(filename = "./figure/Ejercicio4.pdf")
```
...
ej5.g1 <- ggplot(Experimento, aes(x = Dosis, y = Altura, linetype = Complemento, shape = Complemento)) + geom_line() + geom_point()
ej5.g1

ej5.g2 <- ej5.g1 + geom_errorbar(aes(ymin=Altura-Error_Altura, ymax=Altura+Error_Altura), width=.1, position=position_dodge(0.05))
ej5.g2

ej5.g3 <- ggplot(Experimento, aes(x = Dosis, y = Altura, colour = Complemento)) + geom_line(aes(linetype = Complemento)) + geom_point(aes(shape = Complemento)) + geom_errorbar(aes(ymin=Altura-Error_Altura, ymax=Altura+Error_Altura), width=.1, position=position_dodge(0.05)) + theme_classic() + scale_color_brewer(palette="Reds")
ej5.g3

ggsave(filename = "./figure/Ejercicio5.pdf")
```
...
ggplot(ds, aes(x = Year, y = MaxTemp)) + geom_violin(aes(fill = Year)) + labs(title ="Distribución de temperaturas máximas por año", x = "Año", y = "Temperatura (ºC)")

ggplot(ds, aes(x = Year, y = MaxTemp)) + geom_violin(aes(fill = Year)) + labs(title ="Distribución de temperaturas máximas por año", x = "Año", y = "Temperatura (ºC)") + theme(legend.position = "none", axis.text.x = element_text(angle = 45))

ggsave(filename = "./figure/Ejercicio6.pdf")
```
...
ggplot(chol, aes(x = HEIGHT)) + geom_histogram(aes(y=stat(density)), breaks=seq(155, 195, by=2), col="red", fill="blue", alpha=.5) + labs(title="Histograma de la variable Altura", x="Altura (cm)", y="Frecuencia") + geom_density(col="green") + facet_grid(.~SMOKE)

ggsave(filename = "./figure/Ejercicio7.pdf")
```
